...
On write, `Some` merges nested object fields into the parent and `None` omits them. On read, a successful nested object read becomes `Some`; nested `JsError`s are **propagated** (same as non-`Option` `@Flatten`). Missing nested fields therefore fail when the nested `Reads` requires them — `None` is not inferred from absence or invalid nested data.
